...
const Header = () => {
  ...
  return (
    <header className='fixed top-0 left m-auto max_padd_container w-full bg-white ring-1 ring-slate-900/5 z-10'>
      <div className='px-4 flexBetween py-3 max-xs:px-2'>
        <div>
          <Link><img src={logo} alt='Logo' height={66} width={88}></img></Link>
        </div>
        {/* Navbar Desktop */}
        <Navbar containerStyles={"hidden md:flex gap-x-5 xl:gap-x-10 medium-15 capitalize"}/>

        {/* Navbar Mobile */}
        <Navbar containerStyles={`${menuOpened? "flex flexCenter items-start flex-col gap-y-12 fixed top-20 right-8 p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-1 ring-slate-900/5 transition-all duration-300": "flex items-start flex-col gap-y-12 fixed top-20 p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-1 ring-slate-900/5 transition-all duration-300 -right-[100%]"}`}/>
        ...
        <div className='flexBetween sm:gap-x-2 bold-16'>
          ...
          <div className='flexBetween sm:gap-x-6'>
            <NavLink to={'cart-page'} className={"flex"}><FaOpencart className="p-1 h-8 w-8 ring-slate-900/30 rign-1 rounded-full"/>
            ...
            {/* <NavLink to={'logout'} className={"btn_secondary_rounded flexCenter gap-x-2 medium-16"}>
              <img src={logout} alt='logout icon' height={19} width={19}></img> Logout
            </NavLink> */}
            <NavLink to={'login'} className={"btn_secondary_rounded flexCenter gap-x-2 medium-16"}>
              <img src={user} alt='user icon' height={19} width={19}></img>Login
            </NavLink>
          </div>
        </div>
      </div>
